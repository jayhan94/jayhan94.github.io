...
```
当3尚未进入*CANCELLED*状态，即使二者交叉进行，由于终态一致，都是跳过*CANCELLED*节点，并不影响终态条件的达成。
...
Pred <----- Cancelled1             Cancelled2
 ^                                      |
 |                                      |
 +------ Thread3(status!=cancelled) <---+          
```
当3将节点状态更新为*CANCELLED*时，此时4要么已经跳过，要么已经以3为前驱停止跳过。如果是后者，后续如何处理？见下方[cancelAcquire的过程](#cancelacquire的过程)分析。
